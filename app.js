...
function repeatBtnHtml(){
  return '<button class="repeat title-repeat' + (repeatOn ? ' on' : '') + '" id="repeat" type="button" aria-pressed="' + (repeatOn ? 'true' : 'false') + '" aria-label="' + (repeatOn ? 'Repeat one on' : 'Repeat one off') + '" title="' + (repeatOn ? 'Repeat one on' : 'Repeat one') + '">' + repeatIconSvg() + '</button>';
}
function gameItHtml(){
