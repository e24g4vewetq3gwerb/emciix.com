(function () {
  ...
  async function submit(input, status) {
    ...
    const publish = (window.EmciixScores && window.EmciixScores.publishNamedSession) || window.EmciixPublishNamed;
    try {
      if (publish) await publish(name, pts);
      if (status) status.textContent = "Uploaded · " + name + " · " + pts + " PTS";
      window.dispatchEvent(new Event("emciix-ranks-refresh"));
    } catch (err) {
