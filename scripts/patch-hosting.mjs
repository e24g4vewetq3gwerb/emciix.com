...
function materializeHome() {
  ...
  if (!existsSync(src)) return null;
  let html = readFileSync(src, "utf8");
  html = html.replace(/portal-preview\.js\?v=prev-\d+/g, "portal-preview.js?v=prev-4");
  if (!html.includes("portal-preview.js")) {
    html = html.replace("</body>", '  <script src="/portal-preview.js?v=prev-4" defer></script>\n</body>');
  }
...
["video/no-room-for-me.mp4","video/no-room.html","video/tell-me-more.mp4","video/tell-me-more.html","media/drive-map.json","covers/no-room-for-me.jpg","covers/tell-me-more.jpg"] /* full cut */.forEach((file) => {
